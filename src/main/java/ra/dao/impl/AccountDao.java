...
public class AccountDao implements IAccountDao {
    ...
    public Account login(String username, String password) {
        ...
        try {
            CallableStatement call = conn.prepareCall("Select * from Accounts where email=? and password=?");
    ...
    public void save(Account account) {
        ...
        try {
            ...
            if(account.getId()==null){
                // thêm mới
                 call = conn.prepareCall("Insert Into Accounts(full_name, password, email, phone) values (?,?,?,?)");
                ...
                // cật nhật
                call = conn.prepareCall("update  Accounts set full_name=? , phone=?,password=?,address=?,avatar_url=?,status=? where id =?");
                call.setString(1,account.getFullName());
