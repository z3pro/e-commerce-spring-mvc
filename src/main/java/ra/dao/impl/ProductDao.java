...
public class ProductDao implements IProductDao {
    ...
    try {
      ...
      if (product.getId() == null) {
        ...
        call = conn.prepareCall(
            "Insert Into product(name,description,price,image_url,stock,status,catalog_id) values (?,?,?,?,?,?,?)");
        ...
        // cật nhật
        call = conn.prepareCall(
            "update product set name=?, description=? , price= ?,image_url=?, stock=?, status=?, catalog_id=?  where id =?");
        call.setString(1, product.getName());
        call.setString(2, product.getDescription());
        call.setDouble(3, product.getPrice());
        call.setString(4, product.getImageUrl());
  ...
  @Override
  public List<Product> findByCatalog(Long idCatalog) {
    List<Product> list = new ArrayList<>();
    Connection conn = ConnectionDB.getConnection();
    try {
      CallableStatement call = conn.prepareCall("Select * from product where catalog_id =?");
      call.setLong(1, idCatalog);
      ResultSet rs = call.executeQuery();
      while (rs.next()) {
        Product pro = new Product();
        pro.setId(rs.getLong("id"));
  ...
  public List<Product> findByName(String name) {
    ...
    Connection conn = ConnectionDB.getConnection();
    try {
      CallableStatement call = conn.prepareCall("Select * from product where name like ?");
